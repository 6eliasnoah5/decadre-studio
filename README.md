...
├── impressum.html     ← legal subpage (Impressum)
├── datenschutz.html   ← legal subpage (Datenschutz)
├── README.md          ← this file
├── fonts/             ← place the self-hosted .woff2 fonts here
...
- **Andere Schrift** → `--font-display` und `--font-body` tauschen.
  Vergiss nicht, die `@font-face`-Blöcke oben im `<style>` und die
  Dateien in `/fonts/` anzupassen (siehe Abschnitt **Schriften** unten).
...
2. Inhalt des `site/`-Ordners (`index.html`, `impressum.html`,
   `datenschutz.html`, `README.md`, `fonts/`, `videos/`, `images/`) als
   Root des Repos hochladen.
...
Die Seite lädt **keine** Schriften mehr von Google (kein Request an
`fonts.googleapis.com` / `fonts.gstatic.com`, keine IP-Übertragung in
die USA). Stattdessen werden die `.woff2`-Dateien lokal aus `/fonts/`
ausgeliefert, eingebunden über `@font-face` im `<style>`-Block jeder Seite.

**Du musst diese 7 Dateien in den Ordner `site/fonts/` legen:**

| Datei                          | Familie         | Weight |
| ------------------------------ | --------------- | ------ |
| `InterTight-Medium.woff2`      | Inter Tight     | 500    |
| `InterTight-SemiBold.woff2`    | Inter Tight     | 600    |
| `InterTight-Bold.woff2`        | Inter Tight     | 700    |
...
**Woher bekommst du die Dateien?**

1. Gehe auf **[gwfh.mranftl.com](https://gwfh.mranftl.com)**
   (google-webfonts-helper — lädt Google-Fonts als lokale Dateien).
2. Suche nacheinander nach **Inter Tight**, **Inter** und
   **JetBrains Mono**.
3. Wähle pro Familie die oben gelisteten Weights, als Format **woff2**.
4. Lade das ZIP herunter, entpacke es, und benenne die Dateien exakt
   wie in der Tabelle (oder passe die `src`-Pfade in den `@font-face`-
   Blöcken an deine Dateinamen an).
5. Lege alle `.woff2`-Dateien in `site/fonts/`.

Solange die Dateien fehlen, fällt der Browser auf die System-Schriften
zurück (`system-ui`, `Menlo` etc.) — das Layout bricht nicht, sieht aber
noch nicht final aus. Sobald die Dateien liegen, greift automatisch die
richtige Schrift.
